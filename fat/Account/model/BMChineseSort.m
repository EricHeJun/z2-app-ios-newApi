...
+(NSMutableArray*)ReturnSortChineseArrar:(NSArray*)stringArr{
    ...
    for (BMChineseSort *sort in chineseStringsArray) {
        if ([sort.pinYin containsString:AbnormalLetter]) {
            [otherArray addObject:sort];
        }
    }
    [chineseStringsArray removeObjectsInArray:otherArray];
    [chineseStringsArray addObjectsFromArray:otherArray];
